...
import {
  animationIntensityForResonance,
  ComplexityViewMode,
  shouldShowLink,
} from '../../src/function-box/function-box-interaction-visual-state.js';
  ...
  it('filters visible links in basic complexity mode', () => {
    expect(
      shouldShowLink(
        ...
        ComplexityViewMode.Basic,
      ),
    ).toBe(false);

    expect(
      shouldShowLink(
        ResonanceType.Fractured,
        ComplexityViewMode.Basic,
      ),
    ).toBe(true);
  });
