...
export type PressurePlane = 'Material' | 'Spatial' | 'Mental' | 'Social' | 'Causal' | 'RealityAdjacent';

export type SpellSeedStatus = 'Draft' | 'PracticeReady' | 'WitnessRequired' | 'Returned';

export type PuzzleRoomPhase = 'Dormant' | 'Entered' | 'Reading' | 'Casting' | 'Releasing' | 'Resolved' | 'Failed';

export type StoneRoomFailureState = 'PanicHold' | 'NoAnchor' | 'NoReleaseCondition' | 'ForceBreak';

export interface SpellSeed {
  readonly spellSeedId: string;
  readonly name: string;
  readonly functionType: FunctionType;
  readonly status: SpellSeedStatus;
  readonly planes: PressurePlane[];
  readonly operation: string;
  readonly carriers: string[];
  readonly medicine: string;
  readonly poison: string;
  readonly containmentRule: string;
  readonly misuseRisk: string;
  readonly safetyBoundary: string;
  readonly requiredWitnessType: string;
  readonly returnOutput: string;
}

export interface TrialGate {
  readonly gateId: string;
  readonly fromFunction: FunctionType;
  readonly toFunction: FunctionType;
  readonly question: string;
  readonly passCondition: string;
  readonly reversionWarning: string;
  readonly returnOutput: string;
}
...
export interface ContinuityLedgerEntry {
  ...
  readonly functionTested: FunctionType;
  readonly spellSeedId: string;
  readonly held: string[];
  readonly protected: string[];
  readonly almostBecamePrison: string[];
  readonly releaseConditionsNamed: string[];
  readonly witnessConfirmation: string;
  readonly costPaid: string;
  readonly reversionWarning: string;
  readonly returnOutput: string;
  readonly awaitingCollection: string[];
}

export interface AnchorPlateState {
  readonly plateId: string;
  readonly protectionName: string | null;
...
export interface StonePuzzleRoomState {
  readonly roomId: string;
  readonly name: string;
  readonly functionType: FunctionType;
  readonly spellSeedId: string;
  readonly teachingQuestion: string;
  readonly phase: PuzzleRoomPhase;
  readonly pressureMeter: {
    readonly tremor: number;
    readonly seal: number;
    readonly passage: number;
  };
  readonly anchorPlates: readonly AnchorPlateState[];
  readonly witnessRecords: readonly string[];
  readonly failureState?: StoneRoomFailureState;
  readonly returnOutput?: string;
  readonly ledgerEntry?: ContinuityLedgerEntry;
}

export type StoneRoomAction =
  | { readonly type: 'EnterRoom' }
  | { readonly type: 'TestAnchor'; readonly plateId: string }
  | { readonly type: 'NameProtection'; readonly plateId: string; readonly protectionName: string }
  | { readonly type: 'NameReleaseCondition'; readonly plateId: string; readonly releaseCondition: string }
  | { readonly type: 'CastAnchorPulse'; readonly plateId: string }
  | { readonly type: 'ReleaseAnchor'; readonly plateId: string }
  | { readonly type: 'ForceBreak'; readonly plateId: string }
  | { readonly type: 'RecordLedger' };

export function createAnchorPulseSpellSeed(): SpellSeed {
  return {
...
export function createStoneToClayTrialGate(): TrialGate {
  return {
...
export function createRoomThatWouldNotFall(): StonePuzzleRoomState {
  return {
...
export function applyStoneRoomAction(
...
function createAnchorPlate(plateId: string): AnchorPlateState {
  return {
    plateId,
    protectionName: null,
...
function castAnchorPulse(room: StonePuzzleRoomState, plateId: string): StonePuzzleRoomState {
  const plate = findPlate(room, plateId);
  if (!plate?.discovered) return failRoom(room, 'NoAnchor', `Tried to cast before discovering ${plateId}.`);
  if (!plate.protectionName) return failRoom(room, 'PanicHold', `Tried to stabilize ${plateId} without naming what it protects.`);
  if (!plate.releaseCondition) return failRoom(room, 'NoReleaseCondition', `Tried to stabilize ${plateId} without naming a release condition.`);
  ...
  return updatePlate(
    ...
    'Casting',
    `Cast Anchor Pulse on ${plateId}.`,
  );
}

function releaseAnchor(room: StonePuzzleRoomState, plateId: string): StonePuzzleRoomState {
  const plate = findPlate(room, plateId);
  if (!plate?.stabilized) return room;
  ...
  const releasedRoom = updatePlate(
    ...
    'Releasing',
    `Released ${plateId} after stability became useful.`,
  );
...
function recordLedger(room: StonePuzzleRoomState): StonePuzzleRoomState {
  if (room.phase !== 'Resolved') return room;

  const ledgerEntry: ContinuityLedgerEntry = {
...
function updatePlate(
  room: StonePuzzleRoomState,
  plateId: string,
  updater: (plate: AnchorPlateState) => AnchorPlateState,
  phase: PuzzleRoomPhase,
  witnessRecord: string,
): StonePuzzleRoomState {
  return {
    ...room,
    phase,
    anchorPlates: room.anchorPlates.map((plate) => (
      plate.plateId === plateId ? updater(plate) : plate
    )),
    witnessRecords: appendRecord(room, witnessRecord),
  };
}

function failRoom(room: StonePuzzleRoomState, failureState: StoneRoomFailureState, witnessRecord: string): StonePuzzleRoomState {
  return {
    ...room,
    phase: 'Failed',
    failureState,
...
function findPlate(room: StonePuzzleRoomState, plateId: string): AnchorPlateState | undefined {
  return room.anchorPlates.find((plate) => plate.plateId === plateId);
}

function isRoomReadyToResolve(room: StonePuzzleRoomState): boolean {
...
function appendRecord(room: StonePuzzleRoomState, record: string): string[] {
