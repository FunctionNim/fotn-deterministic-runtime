...
function bar(label: string, value: number, width = 20): string {
...
function runStoneRoomAct(): void {
  ...
  let room: StonePuzzleRoomState = createRoomThatWouldNotFall();
...
function applyAndPrint(
  ...
  action: Parameters<typeof applyStoneRoomAction>[1],
): StonePuzzleRoomState {
  ...
  console.log(
    `    [${next.phase.padEnd(10)}] ${label.padEnd(34)}` +
    `  tremor:${fmt(pMeter.tremor)}  seal:${fmt(pMeter.seal)}  passage:${fmt(pMeter.passage)}`,
  );
  return next;
}

function printRoomSnapshot(room: StonePuzzleRoomState): void {
  ...
  console.log(`  Plates  : ${room.anchorPlates.map((p) =>
    `${p.plateId}(discovered:${p.discovered ? 'Y' : 'N'} stabilized:${p.stabilized ? 'Y' : 'N'} released:${p.released ? 'Y' : 'N'})`
  ).join(', ')}`);
...
function printLedger(entry: ContinuityLedgerEntry): void {
...
function solveRoomFresh(): StonePuzzleRoomState {
...
function runContinuationLoopAct(): void {
  ...
  const stateMatch =
    r1.state.districts.starter.pressureLevel === r2.state.districts.starter.pressureLevel &&
