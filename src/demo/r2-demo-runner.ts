import { applyStoneRoomAction, StonePuzzleRoomState, ContinuityLedgerEntry } from '../runtime/playable-pressure.js';
import { runActIFixture, runActIIFixture, ActIFixture, ActIIFixture } from './r2-demo-fixture.js';
...
function printActI(f: ActIFixture): void {
  ...
  for (const step of f.steps) {
    ...
    if (plateId && step.action.startsWith('TestAnchor')) console.log(`\n  ·· ${plateId} ··`);
    console.log(
      `    [${step.phase.padEnd(10)}] ${step.action.padEnd(34)}` +
  ...
  if (fin.returnOutput) console.log(`  Output  : ${fin.returnOutput}`);
...
function printActII(f: ActIIFixture): void {
