...
import { DistrictState, clamp01 } from '../runtime/continuity-state.js';
import { EventType, ResonanceType } from '../runtime/resonance-types.js';

export interface DistrictRuntimeInput {
  readonly district: DistrictState;
  readonly generatedAtTick: number;
}

export interface DistrictRuntimeResult {
  readonly district: DistrictState;
  readonly emittedEvents: ContinuityEvent[];
}

export class DistrictRuntime {
  public update(input: DistrictRuntimeInput): DistrictRuntimeResult {
...
export function evolveDistrict(district: DistrictState): DistrictState {
  ...
  const pressureLevel = clamp01(
    district.pressureLevel + pressureFromMigration - restorationRelief,
  );

  const resonanceStability = clamp01(
    district.environmentStability
