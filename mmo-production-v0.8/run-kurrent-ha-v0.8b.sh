...
COMPOSE_PROJECT_NAME=fotnkurrentv08b
export COMPOSE_PROJECT_NAME
export KURRENT_CERT_DIR="$CERTS"
export FOTN_PROD_TAIL_COUNT=1000
export FOTN_STREAM_ID="prod-blue-kurrent-v0.8-${RUN_ID}"
...
COMPOSE="$ROOT/mmo-production-v0.8/kurrent-secure-cluster-v0.8b.yml"
...
generate_certs() {
  ...
  docker run --rm -v "$CERTS:/certs" -w /certs eventstore/es-gencert-cli:1.0.2 create-ca
  docker run --rm -v "$CERTS:/certs" -w /certs eventstore/es-gencert-cli:1.0.2 create-node -out ./node1 -ip-addresses 127.0.0.1,172.30.240.11 -dns-names localhost,node1.kurrentdb
  docker run --rm -v "$CERTS:/certs" -w /certs eventstore/es-gencert-cli:1.0.2 create-node -out ./node2 -ip-addresses 127.0.0.1,172.30.240.12 -dns-names localhost,node2.kurrentdb
  docker run --rm -v "$CERTS:/certs" -w /certs eventstore/es-gencert-cli:1.0.2 create-node -out ./node3 -ip-addresses 127.0.0.1,172.30.240.13 -dns-names localhost,node3.kurrentdb
