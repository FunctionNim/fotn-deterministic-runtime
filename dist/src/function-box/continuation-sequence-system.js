...
export class ContinuationSequenceSystem {
    functionBoxRuntime = new FunctionBoxRuntime();
    execute(input) {
        ...
        const fracturedCount = currentSlots
            ...
            .length;
        if (fracturedCount > 0) {
            pressureWarnings.push(`${fracturedCount} fractured continuation slot(s) require stabilization.`);
        }
        return {
            slots: currentSlots,
            modifiers,
...
function mergeSlotStates(baseSlots, updatedSlots) {
    return baseSlots.map((slot) => {
        const updated = updatedSlots.find((candidate) => candidate.slotIndex === slot.slotIndex);
        return updated ?? slot;
    });
}
