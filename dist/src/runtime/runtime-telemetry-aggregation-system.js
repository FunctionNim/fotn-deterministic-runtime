...
export var TelemetrySignalState;
(function (TelemetrySignalState) {
    TelemetrySignalState["Healthy"] = "Healthy";
    TelemetrySignalState["Elevated"] = "Elevated";
    TelemetrySignalState["AtRisk"] = "AtRisk";
    TelemetrySignalState["Critical"] = "Critical";
})(TelemetrySignalState || (TelemetrySignalState = {}));
export class RuntimeTelemetryAggregationSystem {
    aggregate(input) {
        const performanceScore = clamp01(1 - input.averageTickMs / 33
            - input.eventQueueDepth / 100000);
        const continuityRiskScore = clamp01(validationRisk(input.validationStatus) * 0.35
...
function signalStateFor(performanceScore, riskScore) {
    if (riskScore >= 0.75 || performanceScore < 0.4)
        return TelemetrySignalState.Critical;
    if (riskScore >= 0.5 || performanceScore < 0.65)
        return TelemetrySignalState.AtRisk;
    if (riskScore >= 0.25 || performanceScore < 0.8)
        return TelemetrySignalState.Elevated;
