...
const DRIFT_HINT = 'Golden snapshot mismatch. If this change is intentional, run: npm run generate-snapshot';
...
function snap(...keys) {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    return keys.reduce((obj, k) => obj?.[k], snapshot);
}
// eslint-disable-next-line @typescript-eslint/no-explicit-any
function man(...keys) {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    return keys.reduce((obj, k) => obj?.[k], manifest);
}
// ─── Manifest integrity ───────────────────────────────────────────────────────
describe('Audit manifest integrity', () => {
    ...
    it('manifest lists two demo acts', () => {
        expect(manifest.demoActs.length).toBe(2);
    });
...
describe('Act I golden snapshot — action sequence', () => {
    ...
    it('all step actions match snapshot in order', () => {
        const snapshotActions = snap('actI', 'steps').map((s) => s.action);
    ...
    it('all step phases match snapshot in order', () => {
        const snapshotPhases = snap('actI', 'steps').map((s) => s.phase);
    ...
    it('all step pressure meters match snapshot exactly', () => {
        const snapshotMeters = snap('actI', 'steps')
